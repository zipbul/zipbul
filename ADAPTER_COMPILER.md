...
**Last sync**: 2026-04-29. **마지막 코드 작업 커밋**: `b55a76f` (Slice 36 — publicExports). Step 10·11·12 모두 ✅. 36 개 thin slice 누적. Section F 의 7 manifest + Section E codegen + Section J CLI 옵션 풀 + Section K watch + Section L 자기 검증 (Item 109·110·111·112·113 풀) + Section G size/sha256 + contentHash + Section C 풀 검증 (pipeline·enum·class·defineAdapter 단수) + Section H 진단 카테고리·다중 에러·file:line:column + Section I tsc composite/references/tsbuildinfo + Item 16 namespace map + Item 25 publicExports + Item 6·45·46·86 + Section M (reader + 충돌 검출) + Step 12 외부 e2e 모두 동작. 책임 단위로 약 115+건 ✅. baseline 1967/139/370.
...
**Step 3b (완료, commit `ae31a7d`)** 는 `ast-node-locator.ts` 와 그 spec 에서 oxc-parser 의 직접 import 를 0 으로 만든 작업이다. Section 0.4 가 본 Step 의 작업 컨텍스트를 담고 있으며, 후속 Step (6·7·8) 의 동형 마이그레이션 작업에서 패턴 참조용으로 유지된다. 본 Step 완료로 Step 5·6·7 이 모두 진입 가능 상태가 되었고 — 이 3개 Step 의 대상 파일들이 `ast-node-locator` 의 `walkChildren`/`getCalleeMethodName` 등 헬퍼를 소비하기 때문이며, 헬퍼 시그니처가 narrow 타입에서 `Node` union 으로 일반화된 것을 받아 소비자 측도 같은 패턴으로 마이그레이션한다. 본 Step 작업 중 wiring 지점에 추가한 임시 캐스팅 (ast-parser.ts:299-303 의 `as AstNodeLocatorCallbacks['...']`, class-metadata-extractor.spec.ts 의 동일 형태) 은 Step 5 (commit `fb75434`) 에서 `AstNodeLocatorCallbacks` 인터페이스가 `Node | null` 형태로 함께 마이그레이션되어 실제로 제거됨. 같은 패턴으로 Step 6 (commit `ad474e2`) 에서 `extractHandlerContextUsages`/`extractHandlerContextOps` 의 시그니처가 `Node` 로 일반화되면서, Step 5 작업 중 ast-parser.ts:308-312 에 새로 추가했던 `as Parameters<typeof X>[0]` 캐스팅도 자연 제거되었다.

**Step 4 (`expression-converter.ts` 의 잔존 oxc import 제거, 완료 commit `2cee2c2`)** 의 본질은 `buildImportMap()` 의 raw `StaticImport` 의존 해소 + 원본 module specifier (`ZIPBUL_IMPORT_SOURCE` IR 에 emit) 의 안정적 획득이었다. 길대시 메인테이너와의 2 라운드 회신 끝에 0.26.1 patch 배포 + cli 합류 (commit `734b6dd`) 로 `CodeRelation.specifier` 가 *resolved 케이스 포함 모든 relation 에 항상 보존* 되도록 결함 수정, 그 위에 (a) `buildImportMap` 을 `extractRelations` 의 `imports` relation 위로 재설계하고 aliased named 검출 (`srcSymbolName !== dstSymbolName && dstSymbolName !== 'default' && dstSymbolName !== '*'`) 을 cli 측 헬퍼로 처리, (b) `ImportInfo.importSource` 를 `relation.specifier` 로 교체, (c) 잔존 `from 'oxc-parser'` 라인 제거하는 작업을 완료했다. cli 측 9 케이스 검증 (external/relative/relative-with-index/re-export/side-effect/aliased/default/namespace/type-only) 으로 specifier 보존이 모든 import 형태에서 동작함이 직접 측정으로 확인됨.
...
**Step 8 (`lib-augment-injector` 의 oxc 직접 import 제거, 완료 commit `470e742`)** — 본 문서 이전 버전의 "JS 산출물 후처리" 서술은 잘못이었다 (코드 직접 확인으로 정정). 실제 동작은: 미들웨어 라이브러리 패키지의 `bin/build/lib-build.ts` 가 `*.ts` 소스 파일들을 순회하면서, 각 파일 내 `defineMiddleware()` export 의 호출 자리에 `__augments` / `__contextOps` 메타를 주입한 *변형된 TS 소스* 를 만들어 일반 TS 컴파일러로 넘긴다. `lib-augment-injector.ts` 가 그 두 단계 (`extractLibAugments` 추출 → `injectAugmentsIntoSource` 변형) 를 담당했고, 파싱은 이미 길대시 `parseSource` 로 이루어지고 있었으며 (line 1, 85), oxc 직접 의존은 narrow 타입 (`OxcFunction`, `CallExpression`, `ArrowFunctionExpression`, `VariableDeclaration`) 의 `import type` 라인뿐이었다. 본 Step 의 작업은 Step 3b·4·5·6·7 과 동일한 패턴 — narrow 타입을 `Node` union 으로 일반화 + 진입부 가드 — 으로 완료되었다. 정책 결정 0건, 별도 설계 0건.
...
- `b972c35` Slice 34: 진단 file:line:column (Item 79)
- `b592180` Slice 35: framework peer dep 검증 (Item 6·46)
- `b55a76f` Slice 36: publicExports (Item 25)
...
- **Item 132 (specifier 보존) → 길대시 0.26.1 patch 배포 + cli 합류 완료 (commit `734b6dd`)**. 1차 회신에서 cli 측이 binding kind enum 노출 요청, 길대시가 `dstSymbolName` 으로 1:1 도출 가능하다며 거절. 2차 회신에서 cli 측이 본 요청의 본질을 정정 — binding kind 가 아니라 *raw module specifier 항상 보존* 이 진짜 필요 (cli 가 `ZIPBUL_IMPORT_SOURCE` IR 에 원본 소스 텍스트 emit). 0.26.0 의 `CodeRelation.specifier` 가 unresolved 케이스에만 보존되어 resolved (상대 경로 / 외부 패키지 해상 / tsconfig paths alias 해상) 시 원본 specifier 가 사라지는 누락이 확인됨 — `dstFilePath` 절대 경로에서 원본 텍스트로의 역변환은 fragile (`./foo` vs `./foo/index`, paths alias, exports map 모두 복원 불가). 길대시가 0.26.1 patch 로 *모든* `imports`/`re-exports`/`type-references`/dynamic import/`require()` relation 에 `specifier` 를 항상 보존하도록 결함 수정 (non-breaking, 추가만). cli 측에서는 catalog 0.26.0 → 0.26.1 합류 후 9 케이스 직접 검증 완료 (external/relative/relative-with-index/re-export/side-effect/aliased/default/namespace/type-only) — `specifier` 가 모든 케이스에서 원본 소스 텍스트로 보존됨을 측정으로 확인. aliased detection (`import { Foo as Bar }` → originalName='Foo') 은 `srcSymbolName !== dstSymbolName && dstSymbolName !== 'default' && dstSymbolName !== '*'` 로 cli 측 헬퍼만으로 해결.

**Step 진입 영향**.
- Step 4 — **차단 해소, 진입 가능**. 0.26.1 합류 완료로 `buildImportMap` 재설계 의 의존성 (specifier 보존) 충족.
...
5. Step 1~8 인프라 정비 단계 완료 (commit 시리즈 `2cee2c2` / `fb75434` / `ad474e2` / `e19ed78` / `470e742` 등). **Step 9 진입 가능** — 회귀 가드 (lint rule 또는 grep spec) + `package.json` catalog 의 `oxc-parser` 항목 + `packages/cli/package.json` dependencies 의 `oxc-parser` 라인 제거. cli 의 `from 'oxc-parser'` 0 매치 달성, 정책 대기 없음.
...
|---|---|---|---|
| 클래스/메서드/데코레이터/생성자/파라미터/heritage/modifiers 추출 | `extractSymbols(parsed)` (이미 활용 중) + 잔존 oxc narrow 타입을 Node union 으로 일반화 | 5 | ✅ `fb75434` |
...
| `ctx.use(KEY)` / `ctx.set(KEY, V)` / `ctx.get(KEY)` 패턴 매칭 | 현재: cli 자체 walker (Node union + `node.type === 'CallExpression'` 가드 + `roots: ReadonlySet<string>` shadow 검사). 후속 리팩토링 후보: `findPattern` ast-grep 으로 흡수 (별도 작업, 본 oxc 제거 목표와 무관). | 6 | ✅ `ad474e2` |
...
132. ✅ **`extractRelations` binding 메타 강화 — 길대시 0.26.1 patch 배포 + cli 합류 + Step 4 적용 완료 (commit `734b6dd` → `2cee2c2`)**. 본 요청의 본질은 binding kind enum 노출이 아니라 *raw module specifier 항상 보존* 이었음. 길대시 0.26.0 의 `CodeRelation.specifier` 가 unresolved 케이스에만 보존되어 resolved (상대 경로 / 외부 패키지 해상 / tsconfig paths alias 해상) 시 원본 specifier 가 사라지는 누락이 0.26.1 patch 로 수정됨. cli 측 검증 9 케이스 (external/relative/relative-with-index/re-export/side-effect/aliased/default/namespace/type-only) 모두에서 `specifier` 가 원본 소스 텍스트 그대로 보존됨을 직접 측정으로 확인 후 Step 4 의 `buildImportMap` 재설계에 적용 완료. aliased detection (`import { Foo as Bar }` → originalName='Foo') 은 `srcSymbolName !== dstSymbolName && dstSymbolName !== 'default' && dstSymbolName !== '*'` 로 cli 측 헬퍼만으로 해결.
