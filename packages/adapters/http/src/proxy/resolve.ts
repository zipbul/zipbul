...
export function resolveProxyInfo(
  ...
  const xffRaw = headers.get(HttpHeader.XForwardedFor);
  ...
  const forwarded = headers.get(HttpHeader.Forwarded);
  if (forwarded !== null) {
  ...
  const proto = headers.get(HttpHeader.XForwardedProto)?.split(',')[0]?.trim()?.toLowerCase() ?? null;
  const rawHost = headers.get(HttpHeader.XForwardedHost)?.split(',')[0]?.trim() ?? null;
  const host = rawHost !== null && validateForwardedHost(rawHost) ? rawHost : null;
  const rawPort = headers.get(HttpHeader.XForwardedPort)?.split(',')[0]?.trim() ?? null;
