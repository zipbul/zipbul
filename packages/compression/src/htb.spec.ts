...
/** Coerce any Uint8Array to one with a concrete `ArrayBuffer` backing — needed
 *  because Bun's typed-array APIs are typed as `Uint8Array<ArrayBuffer>` while
 *  inferred types often resolve to `Uint8Array<ArrayBufferLike>`. */
function toAB(buf: Uint8Array): Uint8Array<ArrayBuffer> {
...
describe('injectGzipPadding (HTB - Heal the BREACH)', () => {
  ...
  function compress(input: Uint8Array): Uint8Array<ArrayBuffer> {
    return toAB(Bun.gzipSync(toAB(input)));
  }
  ...
  it('should produce output that decompresses to the same data as the original', () => {
    ...
    const decompressed = Bun.gunzipSync(toAB(padded));
  ...
  it('should set the FEXTRA flag (bit 2) in the gzip header', () => {
    const compressed = compress(data);
    const padded = injectGzipPadding(compressed, 32);
    expect(at(padded, 3) & 0x04).toBe(0x04);
  });
  ...
  it('should insert XLEN + RFC 1952 subfield after the 10-byte gzip header', () => {
    ...
    expect(at(padded, 12)).toBe(0x5a);
    expect(at(padded, 13)).toBe(0x50);
    // Subfield LEN = padLen = xlen - 4
    const subfieldLen = at(padded, 14) | (at(padded, 15) << 8);
  ...
  it('should preserve ID1, ID2, CM, MTIME, XFL, OS from the original header', () => {
    ...
    expect(at(padded, 0)).toBe(0x1f);
    expect(at(padded, 1)).toBe(0x8b);
    expect(at(padded, 2)).toBe(0x08);
    // MTIME (bytes 4-7), XFL (byte 8), OS (byte 9) unchanged
    for (const i of [4, 5, 6, 7, 8, 9]) {
      expect(at(padded, i)).toBe(at(compressed, i));
    }
  });
  ...
  it('should handle input that already has FEXTRA by appending a new subfield', () => {
    ...
    existingExtra[3] = at(compressed, 3) | 0x04;
    ...
    const newXlen = at(padded, 10) | (at(padded, 11) << 8);
    ...
    expect(at(padded, 12)).toBe(0xAA);
    expect(at(padded, 13)).toBe(0xBB);
  });
});
...
describe('injectZstdPadding (HTB - Skippable Frame)', () => {
  ...
  function compress(input: Uint8Array): Uint8Array<ArrayBuffer> {
    return new Uint8Array(Bun.zstdCompressSync(toAB(input), { level: 3 }));
  }

  it('should produce output that decompresses to the same data', () => {
    const compressed = compress(data);
    const padded = injectZstdPadding(compressed, 32);
    const decompressed = Bun.zstdDecompressSync(toAB(padded));
    expect(Buffer.from(decompressed).toString()).toBe(Buffer.from(data).toString());
  });
