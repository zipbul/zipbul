...
- **§5.2.5** [무표기] §5.2.1의 이행은 **전송 보안의 관측**을 전제한다 — TLS가 상류 프록시에서 종단되는 배포에서는 미들웨어가 전송 보안을 스스로 관측할 수 없어 배포·어댑터가 공급하는 신호가 필요하고, 이를 검사하지 않는 무조건 방출은 §5.2.1 위반이지만 새어나간 STS는 §5.2.4에 따라 UA가 무시하므로 실해는 없다(fail-safe). 업계 구현 전부가 이 트레이드오프로 무조건 방출을 택하고 있다 [RFC 6797 §7.2·§8.1]
