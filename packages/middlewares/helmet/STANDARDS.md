...
- **§5.2.5** [무표기] §5.2.1의 위반은 응답이 **실제로 비보안 전송으로 전달될 때에만** 성립한다 — TLS가 상류 프록시에서 종단되는 배포에서는 미들웨어가 전송 보안을 스스로 관측할 수 없어 이 판정에는 배포·어댑터가 공급하는 신호가 필요하며(§16.1.11), 비보안 전송으로 새어나간 STS는 §5.2.4에 따라 UA가 무시하므로 fail-safe다 [RFC 6797 §7.2·§8.1]
...
- **§16.1.11** [무표기] 요청이 도달한 전송이 보안인지(TLS 종단 여부)의 판정은 HTTP 어댑터·배포 구성의 소관이다 — 이 미들웨어는 어댑터가 공급하는 신호를 소비할 수 있을 뿐 전송을 스스로 관측하지 않는다(§5.2.5) [파생]
