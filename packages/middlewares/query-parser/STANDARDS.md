...
정본 분담: **RFC 3986**은 query를 비계층(non-hierarchical) 데이터로 정의하며 octet 문법·퍼센트 인코딩 일반론만 규정한다(§1). **쌍 분해는 WHATWG URL Standard의 application/x-www-form-urlencoded 파서가 무조건 정본**이다(§2) — 브라우저 폼·URLSearchParams·`qs` 등 모든 주류 파서와의 상호운용 기준이며, 모드 분기 없이 항상 적용된다.
...
- **§2.4** [MUST] 0x2B(`+`)는 퍼센트 디코딩 **전에**(raw byte 단계에서) 0x20(SP)으로 치환한다 — 순서의 귀결로 `%2B`는 리터럴 `+`로 복원되며 공백이 되지 않는다 [WHATWG URL #concept-urlencoded-parser step 3.4→3.5]. 이 치환은 모드 분기 없이 이름·값 모두에 항상 적용된다 — 옵션으로 끌 수 없다.
