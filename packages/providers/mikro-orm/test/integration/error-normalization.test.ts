...
import { Account, PG_URL, MYSQL_URL, describePg, describeMysql, makeOrm, freshSchema } from './helpers';
...
describePg('error normalization (postgres)', () => {
  ...
  test('a duplicate insert raises UniqueConstraintViolationException', async () => {
    ...
    em1.persist(em1.create(Account, { email: 'dup@x.io', name: 'First' }));
    await em1.flush();

    const em2 = orm.em.fork();
    em2.persist(em2.create(Account, { email: 'dup@x.io', name: 'Second' }));
...
describeMysql('error normalization (mysql)', () => {
  let orm: MikroORM;
  beforeAll(async () => {
    orm = await makeOrm(BunMySqlDriver, MYSQL_URL!);
  });
  afterAll(async () => {
    await orm.close(true);
  });
